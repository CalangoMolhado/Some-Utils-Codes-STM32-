...
#define LCD_MO	0 		//DIN
#define LCD_SCK	1 		//CLK
#define LCD_DC	10 		//DO
#define PORT	GPIOC	//GPIO onde est� o display
...
void LCD5110_CS(unsigned char temp)
{
	if (temp) GPIOC->ODR|=1<<LCD_CS;
	else GPIOC->ODR&=~(1<<LCD_CS);
...
void LCD5110_RST(unsigned char temp)
{
	if (temp) GPIOC->ODR|=1<<LCD_RST;
	else GPIOC->ODR&=~(1<<LCD_RST);

}

void LCD5110_DC(unsigned char temp)
{
	if (temp) GPIOC->ODR|=1<<LCD_DC;
	else GPIOC->ODR&=~(1<<LCD_DC);

}

void LCD5110_MO(unsigned char temp)
{
	if (temp) GPIOC->ODR|=1<<LCD_MO;
	else GPIOC->ODR&=~(1<<LCD_MO);

}

void LCD5110_SCK(unsigned char temp)
{
	if (temp) GPIOC->ODR|=1<<LCD_SCK;
	else GPIOC->ODR&=~(1<<LCD_SCK);
}
